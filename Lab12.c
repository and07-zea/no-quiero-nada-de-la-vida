...
uint8_t write = 0;                                  // Bandera para escribir
uint8_t read = 0;                           // Bandera para leer
uint8_t valor_pot = 0;                              // Variable para valor de potenciometro 
uint8_t sleep = 0;                          // Bandera para dormir o despertar
...
void __interrupt() isr (void){
    ...
    if(PIR1bits.ADIF){                              // Fue interrupci?n del ADC
        ...
        if (PORTBbits.RB0 == 0){                    // Fue RB0 
            if (sleep ==0){                 // Est? despierto
                sleep =1; }                 // Mandarlo a dormir
        }
        
        if (PORTBbits.RB1 == 0){                    // Fue RB1 
            if (sleep ==1){                 // Est? dormido
                sleep =0; }                 // Mandarlo a despertar
        }
        
        if (PORTBbits.RB2 == 0){                    // Fue RB2 
            sleep =0;                       // Mandarlo a despertar
            write = 1; }                    // Guardar valor en EEPROM 
